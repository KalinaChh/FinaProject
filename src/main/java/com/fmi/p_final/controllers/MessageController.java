...
public class MessageController {
    ...
    public ResponseEntity<AppResponse<List<MessageDTO>>> getMessagesSentByUser(@PathVariable Long userId) {
        return messageService.getMessagesSentByUser(userId);
    }

    @GetMapping("/user/{userId}/received")
    public ResponseEntity<AppResponse<List<MessageDTO>>> getMessagesReceivedByUser(@PathVariable Long userId) {
        return messageService.getMessagesReceivedByUser(userId);
    }

    @GetMapping("/channel/{channelId}")
    public ResponseEntity<AppResponse<List<MessageDTO>>> getMessagesInChannel(@PathVariable Long channelId) {
        return messageService.getMessagesInChannel(channelId);
    }

    @PostMapping("/send")
    public ResponseEntity<AppResponse<MessageDTO>> sendDirectMessage(
